...
from cse599o_basics.model_utils import softmax, cross_entropy, lr_scheduler, gradient_clipping, silu
from cse599o_basics.train_utils import get_batch, load_checkpoint, save_checkpoint
from cse599o_basics.attention import scaled_attention, MultiHead
...
def run_multihead_self_attention(
    ...
    """
    multihead = MultiHead(d_model, num_heads)
    weights = {
        'w_q': q_proj_weight, 'w_k': k_proj_weight, 'w_v': v_proj_weight, 'w_o': o_proj_weight
    }
    multihead.load_state_dict(weights)
    return multihead.forward(in_features, None)
...
def run_multihead_self_attention_with_rope(
    ...
    multihead = MultiHead(d_model, num_heads, rope)
    weights = {
        'w_q': q_proj_weight, 'w_k': k_proj_weight, 'w_v': v_proj_weight, 'w_o': o_proj_weight
    }
    multihead.load_state_dict(weights)
...
def run_transformer_block(
    ...
    """
    block = TransformerBlock(d_model, num_heads, d_ff, RoPE(theta, d_model // num_heads, max_seq_len))
...
def run_transformer_lm(
    ...
    """
    lm = TransformerLM(vocab_size, context_length, num_layers, 
                       d_model, num_heads, d_ff, rope_theta)
    
    lm.token_embeddings.load_state_dict({
        "weights": weights['token_embeddings.weight']
    })

    for i, layer in enumerate(lm.layers):
        layer.attn.load_state_dict({
