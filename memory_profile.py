...
import numpy as np
import timeit
import torch.cuda.memory as torch_mem
...
def time_train(model: TransformerLM, optim: AdamW, 
    ...
    for step in range(1, w + n + 1):
        ...
        # want to track torch_mem after warmup
        if track:
            torch_mem._record_memory_history(max_entries=100000)
        ...
        # if just forward:
        # if track:
        #     torch_mem._dump_snapshot("forward_memory_profile.pickle")
        #     torch_mem._record_memory_history(enabled=None)
        ...
        # if track full step:
        if track:
            torch_mem._dump_snapshot("large_model.pickle")
            torch_mem._record_memory_history(enabled=None)
